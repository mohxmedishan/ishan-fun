...
const state={mode:"normal",snake:[],apple:null,direction:{x:1,y:0},nextDirection:{x:1,y:0},apples:0,running:false,gameOver:false,timer:null,lastStep:0,skin:"white",fruit:"apple"};
const fruitEmoji={apple:"🍎",banana:"🍌",mango:"🥭",orange:"🍊",strawberry:"🍓"};
const skinNames={white:"White",red:"Red",green:"Green",blue:"Blue",yellow:"Yellow",rainbow:"Rainbow",gradient:"Gradient"};
...
function draw(){if(!canvas.width)return;const c=configs[state.mode];ctx.clearRect(0,0,canvas.width,canvas.height);ctx.fillStyle="#0d0d10";ctx.fillRect(0,0,canvas.width,canvas.height);
  ...
  state.snake.forEach((seg,i)=>{const pad=i===0?3:4;const x=seg.x*c.cell+pad,y=seg.y*c.cell+pad,w=c.cell-pad*2;ctx.fillStyle=skinColor(i);ctx.shadowBlur=i===0?14:0;ctx.shadowColor=ctx.fillStyle;roundRect(ctx,x,y,w,w,Math.min(7,w*.22));ctx.fill();ctx.shadowBlur=0});
